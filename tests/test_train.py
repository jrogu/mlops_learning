...
from train import create_model, prepare_data, get_transforms, Image_dataset
...
class TestModelTraining(unittest.TestCase):
    ...
    def test_image_dataloader(self):
        ...
        train_dataloader, val_dataloader, _ = prepare_data(
            transforms, TRAIN_IMAGES_PATH, CSV_PATH, self.train_ratio, self.batch_size
        )
    ...
    def test_forward_pass(self):
        transforms = get_transforms()
        train_dataloader, _, _ = prepare_data(
            transforms, TRAIN_IMAGES_PATH, CSV_PATH, self.train_ratio, self.batch_size
        )
        ...
        for img, label in train_dataloader:
            self.assertEqual(
                model(img).shape, torch.Size([img.shape[0], self.num_classes])
            )
